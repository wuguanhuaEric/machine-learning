...
#测试
def lwlrTest(testArr, xArr, yArr, k = 1.0):  #loop over all the data points and applies lwlr to each one
    m = shape(testArr)[0]
    yHat = zeros(m)
    for i in range(m):
...
def regularize(xMat):  #regularize by columns
    inMat = xMat.copy()
    inMeans = mean(inMat, 0)  #calc mean then substract it off
    inVar = var(inMat, 0)  #calc variable of Xi then divide by it
