...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):  #toler表示容错率  常数C
    dataMatrix = mat(dataMatIn); labelMat = mat(classLables).transpose()
    ...
    while (iter < maxIter):
        ...
        for i in range(m):
            ...
            #Ei表示误差
            Ei = fXi - float(labelMat[i])  #预测结果和真是结果比对，计算误差
            #对alpha进行优化，同时检查alpha的值满足两个条件
            if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
                ...
                #eta为alpha[j]的最优修改量
                eta = 2.0 * dataMatrix[i, :] * dataMatrix[j, :].T - dataMatrix[i, :] * dataMatrix[i, :].T - dataMatrix[j, :] * dataMatrix[j, :] * dataMatrix[j, :].T
                if eta >= 0: print ("eta >= 0"); continue
                alphas[j] -= labelMat[j] * (Ei - Ej) / eta
                alphas[j] = clipAlpha(alphas[j], H, L)
                #检查alphas[j]是否有轻微改变
                if (abs(alphas[j] - alphaJold) < 0.00001): print ("j not moving enough"); continue
                alphas[i] += labelMat[j] * labelMat[i] * (alphaJold - alphas[j])  #update i by the same amount as j and the update is in the opposite direction
...
class optStruct:  #保存所有重要值，实现对成员变量的填充
    def __init__(self, dataMatIn, classLabels, C, toler, kTup):  #Initialize the strcture with the parameter
        self.X = dataMatIn
...
def selectJ(i, oS, Ei):  #选择第二个alpha值以保证每次优化的最大步长
    maxK = -1; maxDeltaE = 0; Ej = 0
    oS.eCache[i] = [1, Ei]  #input Ei
    validEcacheList = nonzero(oS.eCache[:, 0].A)[0]  #set Ei valid in cache
...
def innerL(i, oS):
    ...
    if ((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or ((oS.labelMat[i] * Ei > oS.tol) and (oS.alphas[i] > 0)):
        ...
        oS.alphas[i] += oS.labelMat[j] * oS.labelMat[i] * (alphaJold - oS.alphas[j])  #update i by the same amount and the update is in the opposite direction
